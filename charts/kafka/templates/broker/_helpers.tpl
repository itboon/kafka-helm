...
{{- if eq .type "NodePort" -}}
  {{- $addr = "$(POD_HOST_IP)" -}}
  {{- $port = "$(echo $KAFKA_EXTERNAL_NODEPORTS | cut -d',' -f$(($(echo $POD_NAME | sed 's/.*-//')+1)))" -}}
...
- name: KAFKA_EXTERNAL_NODEPORTS
  value: "{{ include "broker.fullNodePorts" . }}"
{{- end }}
...
- name: KAFKA_CFG_LISTENER_SECURITY_PROTOCOL_MAP
  ...
  value: CONTROLLER:PLAINTEXT,BROKER:SASL_PLAINTEXT,EXTERNAL:SASL_PLAINTEXT,PLAINTEXT:PLAINTEXT
  {{- else }}
